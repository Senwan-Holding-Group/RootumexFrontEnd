...
const WhTransferDetails = () => {
  ...
  useEffect(() => {
    if (transferDetails) {
      form.reset({
        docDueDate: new Date(transferDetails.docDueDate),
        from: transferDetails.from,
        to: transferDetails.to,
        remark: transferDetails.remark,
        transferLines: [],
      });
      const linesWithLineProperty = transferDetails.transferLines.map(
        (item, index) => ({
          ...item,
          line: index + 1,
        })
                  ...
                  <div className=" w-1/3 space-y-4">
                    <div className="space-y-1  ">
                      ...
                      <span
                        ...
                        {transferDetails?.docDate &&
                          format(new Date(transferDetails.docDate), "PP")}
                      </span>
                    </div>
                    <FormField
                      control={form.control}
...
export default WhTransferDetails;
