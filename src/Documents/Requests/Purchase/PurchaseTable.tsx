...
const PurchaseTable = () => {
  ...
  const {
    ...
    queryFn: () =>
      getPurchaseOrder(
        `/purchase_order?${search.searchKey}=${search.searchValue}&limit=15&page=${currentPage}`,
        setError,
        setTotalPage
      ),
  ...
  return (
    <div className="max-w-full overflow-hidden h-full space-y-2  bg-white  border border-Primary-15  p-4 rounded-2xl">
      ...
      <Search search={search} setSearch={setSearch} menuList={purschaseMenu} />
      <CreatePO/>
      </div>
              ...
              {!poList?.length ? (
                ...
                poList.map((po) => (
                  <tr
                    ...
                    <td className="pr-6 pl-4 py-3">{po.code}</td>
                    <td className="pr-6 pl-4 py-3">
                      <StatusBadge status={po.status} />
                    </td>
                    <td className="pr-6 pl-4 py-3">{po.vendorName}</td>
                    <td className="pr-6 pl-4 py-3">{format(po.docDate, "yyyy-MM-dd")}</td>
                    <td className="pr-6 pl-4 py-3">{format(po.docDueDate, "yyyy-MM-dd")}</td>
                    <td className="pr-6 pl-4 py-3">{numberWithCommas(po.total)}</td>
                  </tr>
                ))
              )}
            </tbody>
