...
const { setError, setTotalPage, totalPage } = useStateContext();
  ...
  const {
    ...
    queryFn: () =>
      getTransfer(
        `/site_transfer_request?${search.searchKey}=${search.searchValue}&limit=15&page=${currentPage}`,
        setError,
        setTotalPage
      ),
          ...
          <table className="w-full  caption-bottom ">
            ...
            <tbody className=" [&_tr:last-child]:border-0 ">
              {!siteTransferList?.length ? (
                ...
                siteTransferList.map((transfer) => (
                  <tr
                    onClick={() =>
                      navigate(
                        `/rootumex/documents/requests/sites-transfer/details/${transfer.docEntry}`
                      )
                    }
