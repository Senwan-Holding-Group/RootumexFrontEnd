...
const columns = [
  ...
  },
  {
    ...
    accessor: "docDate",
    render: (item: Waste) => format(item.document_date, "yyyy-MM-dd"),
  },
];
const WasteTable = () => {
  ...
  const {
    ...
    isError,
    error,
  } = useQuery(getWasteQueryOptions(search, currentPage, setTotalPage));
  return (
    <div className="max-w-full overflow-hidden h-full space-y-2 bg-white  border border-Primary-15 p-4 rounded-2xl">
      ...
      <div className=" h-[calc(100dvh-17.325rem)]">
        <DataTable
          ...
          onRowClick={(waste: Waste) =>
            navigate(
              `/rootumex/documents/requests/waste/details/${waste.doc_entry}`
            )
          }
        />
      </div>
