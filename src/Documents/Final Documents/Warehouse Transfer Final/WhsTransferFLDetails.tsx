...
const WhsTransferFLDetails = () => {
  ...
  useEffect(() => {
    if (transferDetailsFL) {
      form.reset({
        from: transferDetailsFL.from,
        to: transferDetailsFL.to,
        remark: transferDetailsFL.remark,
        transferLines: [],
      });
      const linesWithLineProperty = transferDetailsFL.transferLines.map(
        (item, index) => ({
          ...item,
          line: index + 1,
        })
  ...
  }, [form, transferDetailsFL]);

  const { mutate: editTransferFL, isPending } = useMutation({
    mutationFn: (data: EditTransferFLRequest) =>
      putTransferFL(`/transfer_handheld/${id}`, data),
    onSuccess: () => {
