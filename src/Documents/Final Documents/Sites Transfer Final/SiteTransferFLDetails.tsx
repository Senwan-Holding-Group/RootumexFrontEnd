...
const SiteTransferFLDetails = () => {
  ...
  const form = useForm<EditTransferFLRequest>({
    resolver: zodResolver(EditTransferFLSchema),
    defaultValues: {
      from: "",
      to: "",
      remark: "",
  ...
  const {
    ...
    queryKey: ["siteTransferDetailsFL", id],
    queryFn: () => getTransferDetailsFL(`/site_transfer/${id}`, setError),
  ...
  useEffect(() => {
    if (siteTransferDetailsFL) {
      form.reset({
        from: siteTransferDetailsFL.from,
        to: siteTransferDetailsFL.to,
        remark: siteTransferDetailsFL.remark,
        transferLines: [],
      });
      const linesWithLineProperty = siteTransferDetailsFL.transferLines.map(
        (item, index) => ({
          ...item,
          line: index + 1,
        })
  ...
  }, [form, siteTransferDetailsFL]);
  console.log(docLine);

  const { mutate: editTransfer, isPending } = useMutation({
    mutationFn: (data: EditTransferFLRequest) =>
      putTransferFL(`/site_transfer/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["siteTransferDetailsFL"] });
      form.reset();
      setDialogConfig({
  ...
  const { mutate: closeTransferFL, isPending: isClosing } = useMutation({
    mutationFn: () => postTransferFL(`/site_transfer/close/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["siteTransferDetailsFL"] });
      form.reset();
      setDialogConfig({
  ...
  const onSubmit = async (values: EditTransferFLRequest) => {
    const newValues = {
      ...values,
        ...
        <div className=" h-full bg-white border border-Primary-15 rounded-CS flex flex-col justify-between">
          <DataRenderer isLoading={isFetching} isError={isError}>
            <div className="px-6 py-4 flex gap-x-6 items-center h-[4.5rem] border-b border-Primary-15">
              <Link
                to={"/rootumex/documents/final-docs/sites-transfer-FL"}
              ...
              <span className="text-2xl leading-CS  font-bold  text-RT-Black">
                {siteTransferDetailsFL?.transferNumber}
              </span>
            </div>
                  ...
                  <div className=" w-1/3 space-y-4">
                    <div className="space-y-1  ">
                      ...
                      <span
                        ...
                        {siteTransferDetailsFL?.docDate &&
                          format(new Date(siteTransferDetailsFL.docDate), "PP")}
                      </span>
                    </div>
                    <FormField
...
export default SiteTransferFLDetails;
