...
const SiteTransferFLTable = () => {
  ...
  const {
    ...
    queryKey: ["siteTransferListFL", search.searchValue, currentPage],
    queryFn: () =>
      getTransferFL(
        `/site_transfer?${search.searchKey}=${search.searchValue}&limit=15&page=${currentPage}`,
        setError,
        setTotalPage
      ),
          ...
          <table className="w-full  caption-bottom ">
            ...
            <tbody className=" [&_tr:last-child]:border-0 ">
              {!siteTransferListFL?.length ? (
                ...
                siteTransferListFL.map((transfer) => (
                  <tr
                    onClick={() =>
                      navigate(
                        `/rootumex/documents/final-docs/sites-transfer-FL/details/${transfer.docEntry}`
                      )
                    }
                    ...
                    <td className="pr-6 pl-4 py-3">
                      {format(transfer.docDate, "yyyy-MM-dd")}
                    </td>
                    <td className="pr-6 pl-4 py-3">
                      {format(transfer.docDueDate, "yyyy-MM-dd")}
                    </td>
                  </tr>
                ))
              )}
