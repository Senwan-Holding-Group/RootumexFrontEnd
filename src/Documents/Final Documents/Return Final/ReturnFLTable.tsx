...
const ReturnFLTable = () => {
 const { setError, setTotalPage, totalPage } = useStateContext();
  ...
  const {
    ...
    queryFn: () =>
      getReturnFL(
        `/return?${search.searchKey}=${search.searchValue}&limit=15&page=${currentPage}`,
        setError,
        setTotalPage
      ),
          ...
          <table className="w-full  caption-bottom ">
            ...
            <tbody className=" [&_tr:last-child]:border-0 ">
              {!returnListFL?.length ? (
                ...
                returnListFL.map((returnLine) => (
                  <tr
                    onClick={() =>
                      navigate(
                        `/rootumex/documents/final-docs/return-final/details/${returnLine.docEntry}`
                      )
                    }
