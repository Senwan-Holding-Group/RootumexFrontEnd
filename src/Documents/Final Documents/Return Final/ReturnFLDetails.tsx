...
import { getReturnFLDetails, postReturnFL, putReturnFL } from "@/api/client";
...
const ReturnFLDetails = () => {
  ...
  useEffect(() => {
    if (returnDetailsFL) {
      form.reset({
        docDueDate: new Date(returnDetailsFL.docDueDate),
        warehouseCode: returnDetailsFL.warehouseCode,
        projectCode: returnDetailsFL.projectCode,
        remark: returnDetailsFL.remark,
        poLines: [],
      });
      const linesWithLineProperty = returnDetailsFL.poLines.map(
        (item, index) => ({
          ...item,
          line: index + 1,
        })
