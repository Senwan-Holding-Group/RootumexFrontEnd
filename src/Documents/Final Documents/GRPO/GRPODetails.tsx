...
import { getGRPODetails, postGRPO, putGRPO } from "@/api/client";
...
import { EditGRPORequest, EditGRPOSchema } from "@/lib/formsValidation";
import { Dependencies, Docline } from "@/lib/types";
import { calculateLineTotal, numberWithCommas } from "@/lib/utils";
import {
  faChevronLeft,
  faSpinner,
  faSquareCheck,
...
const GRPODetails = () => {
  ...
  const form = useForm<EditGRPORequest>({
    resolver: zodResolver(EditGRPOSchema),
    defaultValues: {
      warehouseCode: "",
      projectCode: "",
      remark: "",
  ...
  const updateLineQuantity = (line: number, newQuantity: string) => {
    ...
    setdocLine(
      docLine.map((value) => {
        ...
        return {
          ...value,
          recieved_quantity: Math.abs(quantity),
  ...
  useEffect(() => {
    if (grpoDetails) {
      form.reset({
        warehouseCode: grpoDetails.warehouseCode,
        projectCode: grpoDetails.projectCode,
        remark: grpoDetails.remark,
        poLines: [],
      });
      const linesWithLineProperty = grpoDetails.poLines.map((item, index) => ({
        ...item,
        line: index + 1,
      }));
  ...
  const { mutate: editPO, isPending } = useMutation({
    mutationFn: (data: EditGRPORequest) => putGRPO(`/GRPO/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["grpoDetails"] });
      form.reset();
      setDialogConfig({
        title: "GRPO updated successfully!",
        description: "Your GRPO is successfully updated ",
        icon: faSquareCheck,
        iconColor: "text-Success-600",
        variant: "success",
      });
  ...
  const onSubmit = async (values: EditGRPORequest) => {
    const newValues = {
      ...values,
